...
func Load() (*Config, error) {
	cfg := &Config{
		Gateway: GatewayConfig{
			HTTPPort:    getEnvInt("NOMAD_HTTP_PORT", 8080),
			WSPort:      getEnvInt("NOMAD_WS_PORT", 8081),
			Bind:        getEnv("NOMAD_BIND", "loopback"),
			CORSOrigins: getEnvSlice("NOMAD_CORS_ORIGINS", []string{"http://localhost:*"}),
		},
		LLM: LLMConfig{
		...
		Security: SecurityConfig{
			JWTSecret:      getEnv("NOMAD_JWT_SECRET", ""),
			RateLimitRPS:   getEnvInt("NOMAD_RATE_LIMIT_RPS", 10),
			RateLimitBurst: getEnvInt("NOMAD_RATE_LIMIT_BURST", 20),
			AuthMode:       getEnv("NOMAD_AUTH_MODE", "token"),
		},
		AzureDevOps: AzureDevOpsConfig{
			Enabled:      getEnvBool("AZURE_DEVOPS_ENABLED", false),
			Organization: getEnv("AZURE_DEVOPS_ORG", ""),
		...
		Telegram: TelegramConfig{
			...
			AllowFrom: getEnvInt64Slice("TELEGRAM_ALLOW_FROM", nil),
		},
		Tools: ToolsConfig{
			FileRead: FileReadConfig{
				Enabled:          getEnvBool("NOMAD_TOOL_FILE_READ", true),
				AllowedPaths:     getEnvSlice("NOMAD_FILE_ALLOWED_PATHS", []string{"/workspace"}),
				MaxFileSizeBytes: getEnvInt64("NOMAD_FILE_MAX_SIZE", 10*1024*1024), // 10MB
			},
			CommandExecute: CommandExecuteConfig{
				Enabled:         getEnvBool("NOMAD_TOOL_COMMAND_EXEC", false),
				AllowedCommands: getEnvSlice("NOMAD_ALLOWED_COMMANDS", []string{"ls", "cat", "grep", "find"}),
				TimeoutSec:      getEnvInt("NOMAD_COMMAND_TIMEOUT", 30),
			},
			WebSearch: WebSearchConfig{
				Enabled: getEnvBool("NOMAD_TOOL_WEB_SEARCH", false),
				Engine:  getEnv("NOMAD_SEARCH_ENGINE", "duckduckgo"),
				BaseURL: getEnv("NOMAD_SEARCH_URL", ""),
			},
		},
	}
...
func (c *Config) validate() error {
	// Security: require JWT secret in token mode
	if c.Security.AuthMode == "token" && c.Security.JWTSecret == "" {
		return fmt.Errorf("NOMAD_JWT_SECRET is required when auth mode is 'token'")
	}

	// Azure DevOps validation
	if c.AzureDevOps.Enabled {
		if c.AzureDevOps.Organization == "" {
			return fmt.Errorf("AZURE_DEVOPS_ORG is required when Azure DevOps is enabled")
		}
